...
public class TaxpayerManager {
  ...
  public void createTaxpayer(String fullname, int taxRegistrationNumber, String status, float income) throws WrongTaxpayerStatusException{
    TaxpayerFactory factory = new TaxpayerFactory();
    taxpayerHashMap.put(taxRegistrationNumber , factory.generateTaxpayer(fullname, taxRegistrationNumber, status, income));
  }
  ...
  public void createReceipt(int receiptId, String issueDate, float amount, String kind,
    ...
    Receipt receipt = new Receipt(receiptId, issueDate, amount, kind, companyName, country, city, street, number);
  ...
  private void updateFiles(int taxRegistrationNumber) throws IOException {
    ...
    FileWriterFactory fileWriter = new FileWriterFactory();
    fileWriter.updateFiles(taxRegistrationNumber); 
  ...
  public void saveLogFile(int taxRegistrationNumber, String fileFormat)
    ...
    FileWriterFactory fileWriter = new FileWriterFactory();
    fileWriter.generateFileWriter(fileFormat).generateFile(taxRegistrationNumber);
  ...
  public void loadTaxpayer(String fileName)
    ...
    FileReaderFactory reader = new FileReaderFactory();
    reader.generateFileReader(fileName).readFile(fileName);
